...
pub fn import_data_zip(db: &DbState, path: &str, options: &ImportOptions) -> Result<(), AppError> {
    ...
    // 附件文件需要独立处理（释放事务后再写文件）
    let need_attachments = options.include_attachments;
    ...
    if need_attachments {
        ...
        for i in 0..archive_count {
            ...
            let dest_path = data_dir.join(&name);
            ...
            std::io::copy(&mut f, &mut out).map_err(|e| AppError::Io(e.to_string()))?;
        }
    }
