...
- 跨平台系统剪贴板支持和复制/粘贴结果提示
- 表格插入与已有表格调整面板，支持行数、列数和列对齐
- Markdown 富文本渲染覆盖 Mermaid、Flowchart、数学公式、脚注、定义列表和 HTML
- 双语使用文档、故障排查文档和发布验收文档

### Changed

- 摘要输入框改为固定尺寸
- 表格工具栏提示根据光标位置显示“插入表格”或“调整表格”
- 编辑器滚动和图表渲染保持稳定，避免重复初始化和闪烁
- CI 发布流程增加版本、签名、更新清单和发布资产校验

### Fixed

- 修复点击表格面板后编辑器失去光标导致表格无法插入的问题
- 修复已有表格无法调整属性的问题
- 修复 Windows 原生剪贴板历史记录无法识别部分复制内容的问题
- 修复正式构建中图表运行时资源缺失的问题

### Compatibility

- Windows 使用原生剪贴板路径，可配合 `Win + V`
- macOS/Linux 使用平台可用的剪贴板 API，不依赖 Windows Clipboard History
...
- 悬浮球功能：收起时 64x64 球体，展开时 300x300 径向菜单，支持快速笔记、搜索等操作
- 悬浮球设置项：透明度调节、窗口置顶、开关控制，状态持久化
- 快速笔记独立窗口（QuickNotePage），支持 Markdown 编辑
- 完整中英文国际化支持
- 悬浮球相关单元测试
